...
python -c "import psycopg2; conn = psycopg2.connect(host='localhost', port=5433, database='option_trading', user='postgres', password='DB_PASSWORD_REDACTED'); cursor = conn.cursor(); cursor.execute('SELECT MAX(captured_at) FROM historical_trades'); print(cursor.fetchone()[0])"
```
