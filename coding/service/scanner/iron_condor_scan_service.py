"""
Iron condor scanner service. Regime-gated defined-risk complement to
StraddleScanService -- see
...
THE ONE DATA SOURCE RULE (same as StraddleScanService): fetches market data
exclusively via DeribitApiService.get_option_chain_snapshot(currency).
"""
import logging
import math
...
from coding.service.scanner.defined_risk_candidate_builder import build_iron_condor_candidates
...
class IronCondorScanService:
    ...
    def __init__(
        ...
        Args:
            ...
            repository: Injected DatabaseRepository (tests). If None, a
                ...
                -- matching StraddleScanService's own lazy-DB convention.
    ...
    def scan(self, currency: str, regime: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        Returns:
            ...
            -- same top-level shape as StraddleScanService.scan(). Each
        ...
        for expiry, expiry_contracts in contracts_by_expiry.items():
            ...
            candidates = build_iron_condor_candidates(liquid, future_price, sigma_sqrt_t)
            if not candidates:
                excluded.append({"expiry": expiry, "dte": dte, "reason": "no iron condor candidates constructed"})
                continue
    ...
    def _sigma_sqrt_t(self, currency: str, dte: float, as_of: datetime) -> Optional[float]:
        ...
        specific expiry's candidate probability/EV scoring, matching StraddleScanService's
