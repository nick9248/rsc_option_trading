...
class _FakeAnalyzer:
    """Minimal stand-in for OnChainAnalyzer's attributes/methods this
    method reads -- avoids needing a full parsed book summary."""
    ...
    def __init__(self, underlying_price=90000.0, dvol=None, atm_ivs=None, recent_trades=None):
        ...
        self.market_wide_sections = {}
        self.market_wide_structured = {}

    def set_market_wide_section(self, name, text):
        self.market_wide_sections[name] = text

    def set_market_wide_structured(self, data):
        self.market_wide_structured = data


def _price_points(n: int):
    """
    ``n`` daily points ending at "now" (not epoch 0) -- VRPCalculator.
    calculate_realized_volatility filters price_history to a window
    relative to the real ``datetime.now()`` (no reference_time is threaded
    through from this service), so synthetic timestamps must be real-ish or
    every point gets filtered out of the RV window.
    """
...
def _run(api, analyzer) -> "MarketWideResult":  # noqa: F821 (typing only)
...
class TestFundingGate:
    ...
    def test_funding_unavailable_produces_none_not_zero_result(self):
        api = _make_api(funding_8h=None, current_funding=None)
        mw = _run(api, _FakeAnalyzer())

        assert mw.perpetual_funding is None

    def test_funding_available_produces_populated_result(self):
    ...
    def test_only_current_funding_available_funding_8h_stays_none(self):
...
class TestVrpGate:
    ...
    def test_dvol_unavailable_with_sufficient_rv_still_produces_result_with_none_dvol(self):
        api = _make_api(own_price_len=200)
        mw = _run(api, _FakeAnalyzer(dvol=None))

        assert mw.variance_risk_premium is not None
        assert mw.variance_risk_premium.dvol is None

    def test_dvol_available_produces_result_with_dvol_set(self):
        api = _make_api(own_price_len=200)
        mw = _run(api, _FakeAnalyzer(dvol=65.0))
...
class TestVolatilityConeGate:
    ...
    def test_insufficient_price_history_produces_none_not_fake_zero_result(self):
        api = _make_api(own_price_len=20)  # >=11 (RV computes) but <35 (cone insufficient)
        mw = _run(api, _FakeAnalyzer())

        assert mw.volatility_cone is None

    def test_sufficient_price_history_produces_populated_result(self):
        api = _make_api(own_price_len=200)
        mw = _run(api, _FakeAnalyzer())
...
class TestCrossAssetCorrelationGate:
    ...
    def test_insufficient_data_produces_none_correlations_not_zero(self):
    ...
    def test_sufficient_data_produces_real_correlation_values(self):
